...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		$data['import'] = $this->url->link('catalog/product/import', 'token=' . $this->session->data['token'] . $url, true);
	...
	public function import() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/product/import', 'token=' . $this->session->data['token'] . $url, true)
		);
				...
				while(! feof($handle))
  				{
					...
					if($filesop[$j+2] != '')
					{
						...
						{
							if($category['parent_id'] == 0  && strtolower(trim($category['name'])) == strtolower(trim($filesop[$j+2])))
							{
								$catfound = $category['category_id'];
								break;
							}
						}
						...
						if($catfound == 0)
						{
							...
							$catdata['status'] = 1;
							$catdata['category_description'] = array('1' => array('name' => $filesop[$j+2], 'description' => '', 'meta_title' => $filesop[$j+2], 'meta_description' => '', 'meta_keyword' => ''));
							$catdata['category_store'] = array(0);
							$catdata['category_layout'] = array('0' => '0');

							$catfound = $this->model_catalog_category->addCategory($catdata);

							if($filesop[$j+3] != '')
							{
								$subcatdata = array();
								$subcatdata['parent_id'] = $catfound;
								$subcatdata['column'] = 1;
								$subcatdata['sort_order'] = 0;
								$subcatdata['status'] = 1;
								$subcatdata['category_description'] = array('1' => array('name' => $filesop[$j+3], 'description' => '', 'meta_title' => $filesop[$j+3], 'meta_description' => '', 'meta_keyword' => ''));
								$subcatdata['category_store'] = array(0);
								$subcatdata['category_layout'] = array('0' => '0');

								$subcatfound = $this->model_catalog_category->addCategory($subcatdata);
								$category_id = $subcatfound;
							}
							else
							{
								$category_id = $catfound;
							}
						...
						else
						{
							if($filesop[$j+3] != '')
							{
								...
								foreach($categories as $category)
								{
									$cats = array_filter(explode('&nbsp;&nbsp;&gt;&nbsp;&nbsp;', $category['name']));
									if($category['parent_id'] == $catfound && sizeof($cats) == 2 && strtolower(trim($cats[1])) == strtolower(trim($filesop[$j+3])))
									{
										
										$subcatfound = $category['category_id'];
										$category_id = $subcatfound;
									}
								}
					...
					$product_name = $filesop[$j];
					$type = $filesop[$j+1];
					$unit = $filesop[$j+4];
					$brand = $filesop[$j+5];
					$tax = $filesop[$j+6];
					$sku= $filesop[$j+7];
					$image1 = 'catalog/import/'.$filesop[$j+7].'.jpg';
					$description = '';
					$price = 0;
					
				
					$this->model_catalog_product->importExcel($category_id, $product_name, $image1, $type, $unit, $brand, $tax, $sku, $description, $price);
  				}
